...
def show_rounge_page() :
    ...
    br_weekend, cart, receipt, return_b = st.tabs(['Brunch(Weekend/holiday)', 'cart', 'Order Status', '첫 페이지로 돌아가기'])

    with br_weekend:
        # def get_menu(db, column_list, target, target_value) 
        #브런치(주중) 하위 메뉴에 대한 정보 불러오기
        detail = cond.get_menu(db, ['time'], 'menu', 'title_kr', '브런치(주말, 휴일)', True) 
        ...
        with combo :
            # 세트 메뉴에 대한 정보 불러오기
            de_com = cond.get_menu(db, ['name_kr', 'time_weekend_holiday', 'table_name'], 'sub_brunch', 'type', 'combo', True)
        ...
        with promotion :
            # 정보 불러오기
            de_pro = cond.get_menu(db, ['name_kr', 'time_weekend_holiday', 'table_name'], 'sub_brunch', 'type', 'promotion', True)
            # 메뉴의 하위 메뉴에 대한 탭 생성
        ...
        with meal :
            # 세트 메뉴에 대한 정보 불러오기
            de_meal = cond.get_menu(db, ['name_kr', 'time_weekend_holiday', 'table_name'], 'sub_brunch', 'type', 'meal', True)
        ...
        with dessert :
            # 메뉴에 대한 정보 불러오기
            de_dess = cond.get_menu(db, ['name_kr', 'time_weekend_holiday', 'table_name'], 'sub_brunch', 'type', 'dessert', True)
        ...
        with beverage :
            # 세트 메뉴에 대한 정보 불러오기
            de_be = cond.get_menu(db, ['name_kr', 'time_weekend_holiday', 'table_name'], 'sub_brunch', 'type', 'beverage', True)
            # 세트 메뉴의 하위 메뉴에 대한 탭 생성
        ...
        with alcohol :
            # 세트 메뉴에 대한 정보 불러오기
            de_al = cond.get_menu(db, ['name_kr', 'time_weekend_holiday', 'table_name'], 'sub_brunch', 'type', 'alcohol', True)
            # 세트 메뉴의 하위 메뉴에 대한 탭 생성
    ...
    with cart:
        ...
        st.table(cart_final)
        st.button ("Order Now", on_click=order_pressed, args = ('브런치(주말, 휴일)', food_list, qty_list, amt_list))
